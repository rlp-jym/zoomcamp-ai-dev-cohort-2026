...
def immediate_transaction() -> Generator[Session, None, None]:
    ...
    On SQLite this reserves the writer with ``BEGIN IMMEDIATE``, serializing
    claims (and recovery or terminal submissions) across API processes because
    SQLite lacks PostgreSQL's ``FOR UPDATE SKIP LOCKED``.  On PostgreSQL the
    claim query uses ``FOR UPDATE SKIP LOCKED`` instead, so this seam only
    needs a plain transaction.  This module is the isolated storage seam a
    PostgreSQL port replaces.
    """
